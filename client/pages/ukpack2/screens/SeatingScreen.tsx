...
const SeatingScreen: React.FC = () => {
  ...
  const CHASSIS_LABELS: Record<string, string> = {
    ...
    extra: "รถกระบะดัดแปลง 8-12 ที่���ั่ง",
  };
  ...
  const validateSeating = (): boolean => {
    ...
    if (totalSeats < minCapacity) {
      ...
      setErrorMessage(
        `รถประเภทนี้ต้องมีที่นั่งอย่างน้อย ${minCapacity} ที่นั่ง กรุณา��พิ่มจำนวนที่นั่ง`,
      );
    ...
    if (totalSeats > maxCapacity) {
      setErrorTitle("จำนวนที่���ั่งเกินขีดจำกัด");
      setErrorMessage(
        `รถประเภทนี้สามารถมีที่นั่งได้สูงสุ�� ${maxCapacity} ที่นั่ง กรุณาลดจำนวนที่นั่ง`,
      );
      setErrorModalOpen(true);
      return false;
    }
  ...
  const handleTotalSeatsChange = (v: number) => {
    ...
    if (v > maxCapacity) {
      ...
      setErrorTitle("จำนวน���ี่นั่งน้อยเกินไป");
      setErrorMessage(
