...
const SummaryCard: React.FC<SummaryCardProps> = ({ designData }) => {
  ...
  const formatValue = (key: string, value: any) => {
    if (key === "chassis")
      return CHASSIS_LABELS[String(value)] || String(value);
    if (key === "seating") {
      if (!value) return "-";
      if (typeof value === "object")
        return String(value.totalSeats ?? JSON.stringify(value));
      return String(value);
    }
    ...
    if (key === "doors") {
      ...
      if (typeof value === "object")
        return value.doorChoice
          ? value.doorChoice === "1"
            ? "1 ประตู"
            : value.doorChoice === "2"
              ? "2 ประตู"
              : String(value.doorChoice)
      ...
      <div className="grid grid-cols-2 gap-y-2 gap-x-4">
        {entries.map(([k, v]) => (
          <React.Fragment key={k}>
            <div className="font-sarabun text-xs text-gray-500 break-words capitalize">
              {k}
            </div>
            <div className="font-sarabun text-sm font-semibold break-words">
              {formatValue(k, v)}
            </div>
          </React.Fragment>
        ))}
