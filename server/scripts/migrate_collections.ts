...
async function main() {
  ...
  const fromArg =
    args.find((a) => a.startsWith("--from=")) ||
    "--from=minigame1_events/minigame1-di";
  const toArg =
    args.find((a) => a.startsWith("--to=")) ||
    "--to=beforecitychange_events/beforecitychange-di";
  ...
  if (!fromCol || !fromDoc || !toCol || !toDoc) {
    console.error(
      "Invalid from/to format. Use --from=collection/doc --to=collection/doc",
    );
    process.exit(1);
  }

  console.log(
    `Migrating from ${fromCol}/${fromDoc}/events -> ${toCol}/${toDoc}/events`,
  );
